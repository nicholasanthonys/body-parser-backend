...
export default class ContainerController {
  ...
  async store(
    storeContainerDTO: IStoreContainerDTO,
    userId: string
  ): Promise<IContainer> {
  ...
  async show(dbContainerId: string, userId: string): Promise<IContainerCustom> {
    ...
    const dockerContainerInspect = await this.getDockerContainerInspectInfo(
      container.id
    );
  ...
  async update(
    updateContainerDTO: IUpdateContainerDTO,
    userId: string
  ): Promise<IContainerCustom> {
    let container = (await ContainerModel.findOne({
      user_id: userId,
      _id: updateContainerDTO.id,
    })) as IContainer;
    if (!container) {
      throw new Error("Container not found");
    }
    ...
    let running = false;
    dockerContainerInspect = await this.getDockerContainerInspectInfo(
      updatedContainer.container_id
    );
    if (dockerContainerInspect) {
      running = dockerContainerInspect.State.Running;
    }
  ...
  async delete(containerId: string, userId: string) {
    const containerModel = (await ContainerModel.findOne({
      _id: containerId,
      user_id: userId,
    })) as IContainer;
  ...
  async getDockerContainerInspectInfo(
    containerId: string
  ...
  async createContainer(dbContainer: IContainer): Promise<Docker.Container> {
    ...
    //* Set docker network
    let dockerNetwork =
      process.env.DOCKER_NETWORK_NAME || "proxy_middleware_net";
    let EndpointsConfig: { [dockerNetwork: string]: {} } = {};
    EndpointsConfig[dockerNetwork] = {};

    //* Set docker volume
    let dockerContainerVolumeMountpoint =
      process.env.DOCKER_CONTAINER_VOLUME_MOUNTPOINT || "/app/src/configures";
    ...
    return await docker
      ...
      .then(async (container) => {
        let pathToConfigures =
          `${process.env.TMP_PATH}/containers/` +
          dbContainer._id +
          "/configures/.";
        shell.exec(
          `docker cp ${pathToConfigures} ${container.id}:${dockerContainerVolumeMountpoint}`
        );
  ...
  async toggleStartStopContainer(
    dbContainerId: string,
    userId: string
  ): Promise<IContainerCustom> {
    let dbContainer = (await ContainerModel.findOne({
